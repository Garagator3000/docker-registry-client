...
// http.Client.
func New(registryURL, username, password string, log *logrus.Logger) (*Registry, error) {
...
func newFromTransport(registryURL, username, password string, transport http.RoundTripper, log *logrus.Logger) (*Registry, error) {
...
func (registry *Registry) Ping() error {
	url := registry.url("/v2/")
	registry.Log.Debugf("registry.ping url=%s", url)
